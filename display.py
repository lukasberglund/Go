...
class Display():
    def __init__(self, block_width, panel_width, background_color,
                 panel_color):
    ...
    def draw_dividing_line(self):
        pygame.draw.aaline(self.screen, (0, 0, 0), (self.width, 0),
    ...
    def draw_player_points(self, font, board, player_color, y_coord):
        text = "%s points: %s" % (player_color, str(
            board.points[player_color]))
        text = text[0].upper() + text[1:]
        font.render_to(self.screen,
                       (self.width + self.buffer_width // 2, y_coord), text,
                       (0, 0, 0))
